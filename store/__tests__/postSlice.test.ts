...
import postsReducer, { fetchPosts, fetchMorePosts, updatePostField } from '@/store/postsSlice';
...
describe('postsSlice', () => {
    ...
    test('fetchPosts successful', async () => {
        ...
        mock.onGet('http://localhost:3000/posts?_page=1&_limit=5').reply(200, mockPosts);
    ...
    test('updatePostField successful', async () => {
        ...
        await store.dispatch(updatePostField({ post: initialPost, field: 'title', value: 'Updated Title' }));
        const state = store.getState().posts;
